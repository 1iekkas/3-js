  ...
  methods: {
    ...
    changeSkin(index) {
      ...
      this.skin.map((mesh) => {
        ...
        if (
          mesh.name.indexOf("Body") > -1 ||
          mesh.name.indexOf("truck07") > -1
        ) {
          // console.log(mesh);
    ...
    init() {
      ...
      objLoader.load(
        ...
        (obj) => {
          ...
          this.doorGroup = obj.children
            .filter((e) => e.type == "Group")
            .map((c) => c.children.filter((e) => e.name === "Car_door")[0])[0]; //.map(e => e.children)

          this.doorGroup.children.map((e, i) => {
            if (e.name.indexOf("FL") > -1) {
              const btnGeometry = new THREE.CircleGeometry(12, 12); // 生成平面几何
              const btnMaterial = new THREE.MeshLambertMaterial({
                // 生成材质
                color: "#F44000",
                side: THREE.DoubleSide,
              });
              const btnMesh = new THREE.Mesh(btnGeometry, btnMaterial); // 生成平面网格
              Object.assign(btnMesh, {
                name: "handrail",
                index: i,
              });
              btnMesh.position.set(95, 25, 0);
              e.add(btnMesh);
            }
            e.userData = this.doorPosition.filter((d) => d.name === e.name)[0];
    ...
    onDocumentClick(event) {
      ...
      if (this.select && intersects.length > 0) {
        ...
        if (object.name === "handrail") {
          let index = object.index;
          console.log(index);
          const duration = 1000;
          let target = this.doorGroup.children[index];

          let rotate = new TWEEN.Tween(target.rotation)
            .to(
              { y: target.userData.status ? 0 : target.userData.deg },
              duration
            )
            .start();
          rotate.chain(rotate);
          rotate.start();
          target.userData.status = target.userData.status ? 0 : 1;

          //AnimationTimes = duration
          hasUpdate = true;
          setTimeout(() => {
            isAnimation = true;
          }, duration);
