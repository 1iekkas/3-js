...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      objLoader.load("static/car/toyo.gltf", (gltf) => {
        console.log(gltf);
        
        
        let obj = gltf.scene;
        //console.log(box.expandByObject(obj.group))
        obj.scale.set(0.3, 0.3, 0.3);
        //obj.position.set(0, 0, 0);
        // console.log(obj.children);
        this.skin = obj.children;
        let doorFrLeftGroup = new THREE.Group()
        obj.children.filter(mesh => mesh.name.indexOf('DoorFrLeft') !== -1 ).map(e => {
          doorFrLeftGroup.add(e)
        })

        let doorFrRightGroup = new THREE.Group()
        obj.children.filter(mesh => mesh.name.indexOf('DoorFrRight') !== -1 ).map(e => {
          doorFrRightGroup.add(e)
        })
        ...
        doorFrLeftGroup.scale.set(0.3, 0.3, 0.3);
        //console.log(doorFrLeftGroup)
        // 
        /* doorFrLeftGroup.rotation.y = -Math.PI / 3
        doorFrLeftGroup.position.x = 135 * 0.3 //40
        doorFrLeftGroup.position.y = 0
        doorFrLeftGroup.position.z = -24 * 0.3//- max / 2 // -(max - z)// -7.1 */

        doorFrRightGroup.scale.set(0.3, 0.3, 0.3);
        /* doorFrRightGroup.rotation.y = Math.PI / 3
        doorFrRightGroup.position.x = -135 * 0.3
        doorFrRightGroup.position.y = 0
        doorFrRightGroup.position.z = -24 * 0.3 */

        this.scene.add(obj);
        this.scene.add(doorFrLeftGroup)
        this.scene.add(doorFrRightGroup)
